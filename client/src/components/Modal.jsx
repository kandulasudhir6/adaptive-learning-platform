...
export default function Modal({ isOpen, onClose, title, children, maxWidth = 'max-w-lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-xs animate-fade-in">
      <div
        className={`w-full ${maxWidth} bg-gray-900 border border-gray-800 rounded-2xl shadow-xl overflow-hidden transform transition-all`}
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-gray-400 hover:text-gray-300 hover:bg-gray-100 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
