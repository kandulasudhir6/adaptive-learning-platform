    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <p className="text-xs font-semibold text-indigo-600 uppercase tracking-widest mb-1">Student Portal</p>
              <h1 className="text-2xl md:text-3xl font-black text-gray-900">Welcome back, {user?.firstName} 👋</h1>
              <p className="text-gray-500 text-sm mt-1">{selectedCourse?.title || 'Your adaptive learning journey.'}</p>
            </div>
      ...
      {!user?.entranceCompleted && (
        <div className="max-w-6xl mx-auto px-4 mt-6">
          <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-gray-900">Mandatory Diagnostic Exam Required</h3>
                <p className="text-sm text-gray-600 mt-0.5">Complete the AI-powered entrance test to unlock your personalized learning roadmap.</p>
              </div>
            </div>
            <button onClick={onOpenExam}
              className="shrink-0 flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm transition cursor-pointer">
          ...
          {[
            ...
            <button key={id} type="button" onClick={() => handleTabChange(id)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold transition cursor-pointer ${
                activeTab === id ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100'
              }`}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 pb-16">
        ...
        {activeTab === 'courses' && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><BookOpen className="w-5 h-5 text-indigo-500" />Course Modules</h3>
            {!user?.entranceCompleted ? (
              <div className="text-center py-16 bg-white border border-gray-200 rounded-2xl text-gray-400 shadow-sm">
                ...
                <p className="font-medium text-gray-600">Complete the entrance exam to unlock modules</p>
              </div>
            ) : modules.length === 0 ? (
              <div className="text-center py-16 bg-white border border-gray-200 rounded-2xl text-gray-400 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {modules.map((mod, idx) => {
                  ...
                  return (
                    <div key={mod.id} className={`bg-white border rounded-2xl p-5 shadow-sm transition ${isUnlocked ? 'border-gray-200 hover:border-gray-300' : 'border-gray-100 opacity-60'}`}>
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black ${
                            isCompleted ? 'bg-green-50 text-green-600 border border-green-200' :
                            isUnlocked ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-gray-100 text-gray-400'
                      ...
                      {isUnlocked && mod.progress && (
                        <div className="mb-3">
                          ...
                          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full bg-indigo-500 rounded-full transition-all" style={{ width: `${mod.progress.studyPercentage ?? 0}%` }} />
                          </div>
                        </div>
                      )}
                      {isUnlocked && (
                        <div className="flex flex-wrap gap-2">
                          <button type="button" onClick={() => onOpenModule(mod.id)}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-xl transition cursor-pointer">
                          ...
                          {testRequested && !testApproved && (
                            <span className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-500 bg-gray-100 border border-gray-200 rounded-xl">
          ...
          <div>
            {!user?.entranceCompleted ? (
              <div className="text-center py-16 bg-white border border-gray-200 rounded-2xl shadow-sm">
                <Lock className="w-10 h-10 mx-auto mb-3 text-gray-300" />
                <p className="text-gray-600 font-medium">Complete the entrance exam to generate your AI roadmap.</p>
                <button onClick={onOpenExam} className="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition cursor-pointer">Start Diagnostic Exam</button>
              </div>
            ) : roadmapLoading ? (
        ...
        {activeTab === 'enroll' && (
          <div className="max-w-xl">
            <h3 className="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2"><UserCheck className="w-5 h-5 text-indigo-500" />Enrollment & Mentor Selection</h3>
            <p className="text-sm text-gray-500 mb-6">Select your course and choose a faculty mentor to guide your learning journey.</p>
            {enrollSuccess ? (
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5 shadow-sm">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Selected Course</label>
                  <div className="px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-900 text-sm font-medium">{selectedCourse?.title || 'No course available'}</div>
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Choose Faculty Mentor</label>
                    ...
                    {faculties.map((fac) => (
                      <button key={fac.id} type="button" onClick={() => setSelectedFacultyId(fac.id)}
                        className={`w-full text-left p-4 rounded-xl border transition cursor-pointer ${
                          selectedFacultyId === fac.id ? 'border-indigo-400 bg-indigo-50 ring-1 ring-indigo-300' : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'
                        }`}>
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-indigo-100 text-indigo-700 text-xs font-black flex items-center justify-center border border-indigo-200">
                            {(fac.firstName || 'F')[0]}{(fac.lastName || 'M')[0]}
                          </div>
                          <div>
                            <div className="font-bold text-gray-900 text-sm">{fac.firstName} {fac.lastName}</div>
                            <div className="text-xs text-gray-400">{fac.email}</div>
                            {fac.subjects?.length > 0 && (
                              <div className="flex gap-1 flex-wrap mt-1">
                                {fac.subjects.slice(0, 3).map((s, i) => (
                                  <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 border border-gray-200">{s.subject_name}</span>
                                ))}
                              </div>
                            )}
                          </div>
                ...
                <button type="button" onClick={handleEnroll} disabled={enrolling || !selectedFacultyId}
                  className="w-full py-3 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer text-sm">
