...
export const sendOtpEmail = async (toEmail, otp) => {
  ...
  if (!RESEND_API_KEY) {
    console.warn('?? RESEND_API_KEY not set. Skipping real email transmission.');
    return false;
  }
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        <div style="font-family: sans-serif; max-w-md; margin: auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 10px; background-color: #f9fafb;">
          ...
          <div style="margin: 30px 0; padding: 20px; background-color: #ffffff; border-radius: 8px; text-align: center; border: 2px dashed #a78bfa;">
            <span style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #4c1d95;"></span>
          </div>
    ...
    console.log(?? Resend email sent successfully to . ID: );
    return true;
  } catch (error) {
